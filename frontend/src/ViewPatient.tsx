...
export const ViewPatient = () => {
    ...
    const { data, status } = useQuery({
        ...
        queryFn: async (): Promise<PatientReports[]> => {
          const data = await fetch(`${import.meta.env.VITE_API_URL}/query/${patientId}`, {
            headers: {
            ...
            <Card className="w-full">
                ...
                <Accordion type="single" collapsible className="p-8">
                    ...
                    {Object.entries(groupedData).map(([diagnosisCode, reports]) => (
                        <AccordionItem key={diagnosisCode} value={`formField-${diagnosisCode}`}>
                            <AccordionTrigger>{diagnosisCode} - {reports[0]?.i_text_dg}</AccordionTrigger>
                            <AccordionContent>
                            <Table>
                                <TableHeader>
                                <TableRow>
                                ...
                                <TableBody>
                                {reports?.map((row) => (
                                    <TableRow key={row.ic_amb_zad} style={{ cursor: 'pointer' }}>
                                    <TableCell className="font-medium">{row.ic_amb_zad}</TableCell>
                                    <TableCell className="font-medium">{row.i_dg_kod}</TableCell>
                                    <TableCell className="font-medium">{row.i_text_dg}</TableCell>
                                    {/* <TableCell>
